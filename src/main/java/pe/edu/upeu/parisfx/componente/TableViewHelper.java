...
public class TableViewHelper<T> {
        ...
        for (Map.Entry<String, ColumnInfo> entry : columns.entrySet()) {
            ...
            if (field.contains(".")) {
                ...
                column.setCellValueFactory(cellData -> {
                    ...
                    try {
                        // Obtener el valor del campo anidado utilizando reflexión
                        Object nestedValue = getNestedFieldValue(item, fieldPath);
                        return new SimpleObjectProperty<>(nestedValue != null ? nestedValue : "N/A");
                    } catch (LazyInitializationException e) {
                        return new SimpleObjectProperty<>("No disponible");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    private Object getNestedFieldValue(T item, String[] fieldPath) throws Exception {
        Object currentObject = item;

        for (String field : fieldPath) {
            // Construir el método getter de la propiedad (por ejemplo, "getNombre" para "nombre")
            String methodName = "get" + capitalize(field);
            Method method = currentObject.getClass().getMethod(methodName);

            // Invocar el método y obtener el siguiente objeto
